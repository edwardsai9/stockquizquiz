...
const items = [
  {
    name: 'leverage',
    prompt: "A playful 3D golden metallic monkey statue, holding two gold dice, glowing orange topaz eyes, hyper-detailed, masterpiece, professional 3D CGI render, isolated on a solid pure black background. ### shadow, floor, gradient, table, pedestal, ground"
  },
  {
    name: 'short',
    prompt: "A powerful 3D golden metallic jaguar statue roaring, red ruby eyes, surrounded by subtle red neon lightning, hyper-detailed, masterpiece, professional 3D CGI render, isolated on a solid pure black background. ### shadow, floor, gradient, table, pedestal, ground"
  },
  {
    name: 'value',
    prompt: "A wise 3D golden metallic owl statue, holding a golden magnifying glass, glowing amethyst purple eyes, hyper-detailed, masterpiece, professional 3D CGI render, isolated on a solid pure black background. ### shadow, floor, gradient, table, pedestal, ground"
  }
];
...
async function main() {
  console.log("Submitting final solid black background image generation jobs (monkey, jaguar, owl) to AI Horde...");
  const jobs = [];
      ...
      try {
        ...
        if (status.done) {
          ...
          if (status.generations && status.generations.length > 0) {
            ...
            // Save raw image directly (NO background removal)
            if (imgData.startsWith('http')) {
              await downloadFile(imgData, job.outFile);
              fs.copyFileSync(job.outFile, job.rawFile);
  ...
  console.log("\nAll remaining black background images regenerated and saved directly successfully!");
}
