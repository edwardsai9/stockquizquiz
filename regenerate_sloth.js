...
const item = {
  name: 'passive',
  prompt: "A luxury 3D golden metallic sloth statue, sitting upright, glowing sapphire blue eyes, hyper-detailed, masterpiece, professional 3D CGI render, isolated on a solid green screen background. ### shadow, floor, gradient, table, pedestal, ground, branch, tree, leaves"
};
...
async function removeBackgroundGreen(inputPath, outputPath) {
  try {
    ...
    function isGreenScreen(r, g, b) {
      if (g > 65 && g > r * 1.15 && g > b * 1.15) return true;
    ...
    console.log(`[Green Sloth] Saved ${outputPath}. Transparent pixels: ${transparentCount} (${((transparentCount / (width * height)) * 100).toFixed(1)}%)`);
  } catch (err) {
    console.error(`Error keying green screen for sloth:`, err);
  }
}

async function main() {
  console.log("Submitting standalone green-screen sloth job to AI Horde...");
  try {
    ...
    while (!done) {
      ...
      if (status.done) {
        ...
        if (status.generations && status.generations.length > 0) {
          ...
          console.log("Keying out green background for sloth...");
          await removeBackgroundGreen(rawFile, outFile);
          done = true;
        }
      } else {
    ...
    console.log("Standalone sloth image generated and keyed out successfully!");
  } catch (err) {
