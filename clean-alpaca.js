...
async function cleanAlpaca() {
  try {
    const image = sharp('res_dividend_raw.png');
    ...
    .png()
    .toFile('res_dividend.png');

    console.log(`Cleaned alpaca: saved res_dividend.png. Transparent pixels: ${transparentCount} (${((transparentCount / (width * height)) * 100).toFixed(1)}%)`);
  } catch (err) {
    console.error(err);
  }
}
